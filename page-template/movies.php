    ...
    <div class="botgrapper_wrp mr-top30">  
        ...
        <div class="row">
            <div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
                <h3><?php the_field('movies', 171); ?></h3>
...
//                                $the_query = new WP_Query($arg);
                            ...
                            $sql = $wpdb->get_results("SELECT af_terms.term_id, af_terms.name  from af_term_taxonomy join af_terms on af_terms.term_id = af_term_taxonomy.term_id where af_term_taxonomy.taxonomy = 'categorie' ");
                            foreach ($sql as $r) {
//                               " term_id" ='term_taxonomy_id'
                    ...
                    <div class="panel-body">
                        <div class="tab-content">
                            ...
                            <?php
                            $f = true;
                            $sql = $wpdb->get_results("SELECT af_terms.name ,af_terms.term_id from af_term_taxonomy join af_terms on af_terms.term_id = af_term_taxonomy.term_id where af_term_taxonomy.taxonomy = 'categorie'  ");
                            foreach ($sql as $r) {
//                                           
                                ?>
        ...
        max(if(`meta_key`='movie_content_0_movie_duration', `meta_value`, null )) AS 'duration',
        max(if(`meta_key`='movie_content_0_movie_metascore', `meta_value`, null )) AS 'metascore',
        max(if(`meta_key`='movie_content_0_movie_description', `meta_value`, null )) AS 'description', 
        max(if(`meta_key`='movie_content_0_movie_director', `meta_value`, null )) AS 'director', 
        max(if(`meta_key`='movie_content_0_movie_stars', `meta_value`, null )) AS 'stars', 
        max(if(`meta_key`='movie_content_0_movie_buzz', `meta_value`, null )) AS 'buzz', 
        max(if(`meta_key`='movie_content_0_movie_tralier', `meta_value`, null )) AS 'trailer', 
...
<a href="<?php echo get_permalink($s->id); ?>">
                                        ...
                                        ?>
                                    <?php } }else{ ?>
 <h4 class="red-color text-uppercase mr-top0 text-center">Oops, no result found !</h4>
                                    <?php }  ?>
                                    <hr>
                                </div>
                            <?php } ?>
